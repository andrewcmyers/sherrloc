...
(* ###################### *)
let rec xmlToDocHelp attList =
  match attList with
    []		-> empty
  | hd::tl  -> 
      match hd with
        (s1, s2)	-> 
          group[text s1 ; br ; text "=" ; br ; text s2]
...
let rec xmlToDoc xml = 
  match xml with
    Element (s, s1, lst)	-> 
    (
      let dc = group[nest 2 [text "<" ; br ; text s ; br ; xmlToDocHelp s1 ;
							br ; text ">" ]] in
      ...
      let dc3 = smartcons(dc, dc2) in
      smartcons( dc3, group[nest 2 [text "</" ; br ; text s ; br ; text ">" ]] ) 
    )
(******************** pretty-printer test *************)
