...
//        }
    ...
    public String genCutItems () {
    	...
    	if (GEN_CUT) {
        	...
        	sb.append("Likely wrong constraints in the source code:\n");
        	sb.append("<UL>\n");
...
//       	            		Position p = new Position(c.getEquation().getInfo());
       	            		if (p.getLine()==currentline) {
       	            			current = current.substring(0, p.getColStart()) + 
       	            				...
       	            				c.getEquation().toHTMLString() +
       	            				"</span>" + 	           
       	            				"<span class=\"wrongConstraint\" onmouseover=\"display_info('wrongcons"+count+"')\" onmouseout=\"hide('wrongcons"+count+"')\">" + 
