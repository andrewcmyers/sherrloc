...
public class TestAll {
     ...
     *
     * @param filename      Constraint inputs
     * @param expectederror True if the input constrains error
     */
     ...
     *
     * @param filename Constraint inputs
     * @param loc      Location of the true mistake
     */
     ...
     *
     * @param filename Constraint inputs
     * @param expected The true missing hypothesis
     */
    public void testAssumptions(String filename, String expected) {
        try {
            ...
            String result = ana.toConsoleString();
            assertTrue("Expected (" + expected + "), but got (" + result + ")",
    ...
    public void regressionTests() {
        ...
        testAssumptions("tests/jif/axiom3.con",
                "- Bool <= Char;Bool <= Collects2;Char <= Bool;Char <= Collects2;\n");
        testAssumptions("tests/jif/axiom4.con",
                "- Bool <= Char;Char <= Bool;cons_2 (Char) (x) <= G;\n");
        ...
        testAssumptions("tests/jif/associativity.con",
                "- (list (x))->(list (x)) <= list (CHAR);list (CHAR) <= (list (x))->(list (x));\n");
...
//		testAssumptions("/home/zhdf/workspace/LemonTool/error.con", "- a <= b\n");
    ...
    public void testHypothesis() {
        ...
        /* auction */
        testAssumptions("tests/hypothesis/constraints/AirlineAgent1.con",
                "- (TheAirline)->(TheAirline) <= C_L;\n");// same assumption
        /* AirlineAgent2 secure */
        /* User1 secure */
        testAssumptions("tests/hypothesis/constraints/AirlineExample1.con",
                "- AirlineA <= airlines;\n");// same assumption
        testAssumptions("tests/hypothesis/constraints/AirlineExample2.con",
                "- AirlineA <= airlines;AirlineB <= airlines;\n");// same assumption
        /* jif compiler is too conservative in this case. it's secure */
        testAssumptions("tests/hypothesis/constraints/AirlineExample3.con",
                "The program passed program analysis. No errors were found."); // secure

        /* battleship */
        testAssumptions("tests/hypothesis/constraints/Board1.con",
                "- (p1)->(p1) <= C_L;(p1)<-(p1) <= I_L;C_L <= (p1)->(p1);I_L <= (p1)<-(p1);\n"); // same assumption

        /* social */
        /* SocialNetwork1 secure */
        testAssumptions("tests/hypothesis/constraints/SocialNetwork2.con",
                "- bb <= SN;bg <= SN;user <= SN;\n"); // same

        /* friendmap */
        testAssumptions("tests/hypothesis/constraints/Location1.con",
                "- C_L <= (C_A)join((⊥)->(⊥));\n");// same assumption
        testAssumptions("tests/hypothesis/constraints/Snapp1.con",
                "- owner2 <= store1;\n"); // same assumption
        /* Snapp2 secure */
        testAssumptions("tests/hypothesis/constraints/FriendMap1.con",
                "- C_*l <= (⊤)->(s1);\n"); // same assumption
        testAssumptions("tests/hypothesis/constraints/FriendMap2.con",
                "- (⊤)->(user.friends) <= (⊤)->(worker$);\n"); // weaker assumption
        testAssumptions("tests/hypothesis/constraints/FriendMap3.con",
                "- C_*map_update <= C_*map_access;\n"); // weaker assumption
        /* FriendMap4 secure */
        testAssumptions("tests/hypothesis/constraints/FriendMap5.con",
                "- C_*l <= (⊤)->(s1);\n"); // weaker assumption
        /* FriendMap6 secure */
        testAssumptions("tests/hypothesis/constraints/FriendMap7.con",
                "- C_l <= (⊤)->(n1);C_n <= (⊤)->(n1);\n"); // same assumption
        testAssumptions("tests/hypothesis/constraints/FriendMap8.con",
                "- C_s <= C_*l;I_s <= I_*l;\n"); // weaker assumption
        testAssumptions("tests/hypothesis/constraints/FriendMap9.con",
                "- C_*iterLabel <= (⊤)->(fn);\n"); //  weaker assumption, this is a good example
        testAssumptions("tests/hypothesis/constraints/FriendMap10.con",
                "- C_*lbl <= (⊤)->(n1);\n"); // weaker  assumption
        testAssumptions("tests/hypothesis/constraints/FriendMap11.con",
                "- C_user <= (⊤)->(n1);\n"); // weaker  assumption
        /* FriendMap12 secure */
        /* FriendMap13 secure */
        testAssumptions("tests/hypothesis/constraints/FriendMap14.con",
                "- C_*iterLabel <= (⊤)->(fn);\n"); // weaker  assumption
        testAssumptions("tests/hypothesis/constraints/FriendMap15.con",
                "C_*iterLabel <= (⊤)->(fn);\n"); // same  assumption
        testAssumptions("tests/hypothesis/constraints/FriendMap16.con",
                "- (⊤)<-(localStore) <= I_*lbl;C_*lbl <= (⊤)->(localStore);\n"); // same  assumption
        testAssumptions("tests/hypothesis/constraints/FriendMap17.con",
                "- C_*l <= (⊤)->(s1);\n"); // same  assumption
        testAssumptions("tests/hypothesis/constraints/Box1.con",
                "- C_L <= (C_A)join((⊥)->(⊥));\n"); // same assumption
        /* Box2 secure */
        /* Box3 secure */
        testAssumptions("tests/hypothesis/constraints/Box4.con",
                "- C_*l <= C_*a;I_*l <= I_*a;\n"); // same assumption
        /* MapImage1 secure */
        testAssumptions("tests/hypothesis/constraints/MapImage2.con",
                "- C_A <= (⊤)->(s4);\n"); // same assumption
        testAssumptions("tests/hypothesis/constraints/MapImage3.con",
                "- C_*a <= (⊤)->(s1);\n"); // same assumption
        testAssumptions("tests/hypothesis/constraints/MapImage4.con",
                "- C_*bdry_update <= C_*bdry_access;I_*bdry_update <= I_*bdry_access;\n"); // same assumption
        testAssumptions("tests/hypothesis/constraints/MapImage5.con",
                "- C_boundary <= C_L;C_data <= C_L;I_boundary <= I_L;I_data <= I_L;\n"); // weaker assumption
        testAssumptions("tests/hypothesis/constraints/MapImage6.con",
                "- C_L <= C_*l;C_a <= C_*l;C_l <= C_*l;I_L <= I_*l;I_a <= I_*l;I_l <= I_*l;\n"); // weaker assumption (6 assumptions with integrity)
        testAssumptions("tests/hypothesis/constraints/MapServer1.con",
                "- C_*l <= (⊤)->(this.store);\n"); // same assumption
    }

    @Test
    public void testFriendMap() {
        testAssumptions("tests/friendmap/constraints/FriendMap3108_1.con",
                "(⊥)<-(⊥) <= I_*map_access");
        testAssumptions("tests/friendmap/constraints/FriendMap3110_1.con",
                "(⊤)<-(user.p) <= I_*map_update");
        testAssumptions("tests/friendmap/constraints/FriendMap3112_1.con",
                "(⊤)<-(local) <= I_*box_u");
        testAssumptions("tests/friendmap/constraints/FriendMap3113_1.con",
                "C_map <= C_*map_update;I_map <= I_*map_update;");
        testAssumptions("tests/friendmap/constraints/FriendMap3114_1.con",
                "C_*map_update <= (⊥)->(⊥)");
        testAssumptions("tests/friendmap/constraints/FriendMap3115_1.con",
                "C_*map_update <= (⊥)->(⊥)");
        testExpression("tests/friendmap/constraints/FriendMap3116_1.con",
                "FriendMap3116.fab:429,32-42");
        testExpression("tests/friendmap/constraints/FriendMap3120_1.con",
                "FriendMap3120.fab:494,4-5");
        testAssumptions("tests/friendmap/constraints/FriendMap3122_1.con",
                "(⊤)<-((fo)meet(fn)) <= I_*map_update");
        testExpression("tests/friendmap/constraints/FriendMap3144_1.con",
                "FriendMap3144.fab:445,32-42");
        testAssumptions("tests/friendmap/constraints/FriendMap3167_1.con",
                "C_l <= (⊤)->(n1);C_n <= (⊤)->(n1)");
        testAssumptions("tests/friendmap/constraints/FriendMap3192_1.con",
                "C_s <= C_*friend_access_bound");
        testAssumptions("tests/friendmap/constraints/FriendMap3193_1.con",
                "(⊤)<-(this.service) <= I_*fetchLabel");
    }
...
//		testConstraint("tests/downgrade/AirlineExample2.con", "AirlineExample2.jif:71,51-57");
        ...
        testConstraint("tests/downgrade/jpmail/constraints/AESClosure_1.con",
                "AESClosure.jif:17,11-30");
        testConstraint("tests/downgrade/jpmail/constraints/DESClosure_1.con",
                "DESClosure.jif:27,11-30");
        testConstraint("tests/downgrade/jpmail/constraints/MD5Closure_1.con",
                "MD5Closure.jif:18,27-36");
        testConstraint("tests/downgrade/jpmail/constraints/RSAClosure_1.con",
                "RSAClosure.jif:20,11-30");
        testConstraint("tests/downgrade/jpmail/constraints/TripleDESClosure_1.con",
                "TripleDESClosure.jif:17,11-30");
        // RSA.jif is secure after removing all downgrades

        // Tests from jpmail
        testConstraint("tests/downgrade/jpmail/constraints/MailReaderCrypto_1.con",
                "MailReaderCrypto.jif:634,24-32");
    }
}
