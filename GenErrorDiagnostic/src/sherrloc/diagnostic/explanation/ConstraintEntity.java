...
public class ConstraintEntity extends Entity {
     ...
     * @param succ # satisfiable paths using the constraint
     */
    public ConstraintEntity(Constraint cons, String pos, String posWithExp, String html,
            String console, int succ, int weight) {
        super(succ, weight);
        this.cons = cons;
        this.pos = pos;
    ...
    public boolean explains(ConstraintPath p) {
        for (Edge edge : p.getEdges()) {
            if (edge instanceof ConstraintEdge && ((ConstraintEdge) edge).getConstraint().getPos()
                    .toString().equals(pos)) {
                return true;
            }
        }
