...
/**
 * Searches for sets of Entity that maximize the term C1*|E|+C2*k_E, where |E| is set size and k_E
 * is # successful paths that use entity in E
 */
public class EntityExplanationFinder extends HeuristicSearch {
    ...
    /**
     * @see #EntityExplanationFinder(UnsatPaths, Entity[], int, double, double). Use default value
     * in {@link RankingMetric}
     */
    ...
    /**
     * @param paths      Unsatisfiable paths identified in constraint analysis
     * @param candidates A set of entities so that all subsets can be a potential explanation
     * @param C1         Value of C1 in ranking metric C1*|E|+C2*k_E
     * @param C2         Value of C2 in ranking metric C1*|E|+C2*k_E
     */
    public EntityExplanationFinder(UnsatPaths paths, Entity[] candidates, int nSubopt, double C1,
            double C2) {
        super(candidates, paths, nSubopt);
        Set<String> candStr = new HashSet<String>();
        for (Entity en : candidates) {
    ...
    /**
     * A heuristic that estimates the "cost" of satisfying remaining paths. The estimation is
     * "consistent", in a sense that it never overestimate the actual cost. This property ensures
     * that the A* search is optimal: all and only the most likely wrong subsets of entities are
     * returned. See this <a href="http://hdl.handle.net/1813/33742">technical report</a> for more
     * details
     *
     * @param paths Remaining unsatisfiable paths
     * @param index The index of the next entity to search
