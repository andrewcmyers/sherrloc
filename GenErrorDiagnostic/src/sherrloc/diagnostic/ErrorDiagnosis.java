...
public class ErrorDiagnosis implements PrettyPrinter {
    ...
    /**
     * @param g      A constraint graph construction from constraint file
     * @param option A configuration of the error diagnostic tool
     */
    private ErrorDiagnosis(ConstraintGraph g, DiagnosticOptions option) {
        graph = g;
        this.option = option;
        util = new HTMLUtil();
        cana = new ConstraintAnalysisImpl(option.isGenHypothesis(), option.isVerbose(),
                option.isRecursive());
    }
    ...
    /**
     * Create an error diagnosis instance given the constraint checker `cc`.
     */
    static public ErrorDiagnosis getAnalysisInstance(ConstraintCheck cc) {
        ConstraintGraph graph = cc.freshGraph();
        graph.generateGraph();
        return new ErrorDiagnosis(graph, cc.options());
    }

    /**
     * Create an error diagnosis instance given the graph `g` and mode `m`.
     */
    static public ErrorDiagnosis getAnalysisInstance(ConstraintGraph g, Mode m) {
        return new ErrorDiagnosis(g, new DiagnosticOptions("", m));
    }
    ...
    /**
     * Use a error diagnosis algorithm as specified in configuration to find the most likely cause
     * of the error
     *
     * @param paths A set of unsatisfiable paths
    ...
    public String getSuggestions(UnsatPaths paths) {
        ...
        if (option.isGenConstraints()) {
            sb.append((new ConstraintInfer(paths, graph.getAllEdges(), option)).infer());
        }
    ...
    public DiagnosticConstraintResult getConstraintResult() {
        ...
        if (paths.size() > 0) {
            return new DiagnosticConstraintResult(false,
                    new ConstraintInfer(paths, graph.getAllEdges(), option).inferResult());
        } else {
    ...
    public String toHTMLString() {
        ...
        if (paths.size() == 0) {
            ...
            if (option.getSourceName() != null) {
                sb.append(
                        util.genAnnotatedCode(paths, option.getSourceName(), option.isGenElements())
                                + (option.getSourceName().contains("jif")
                                ? ("<script>colorize_all(); numberSuggestions();</script>\n")
