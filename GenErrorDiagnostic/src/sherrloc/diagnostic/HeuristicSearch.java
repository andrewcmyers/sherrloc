...
/**
 * This class implements the main functionality of A* search. Given a set of entities, method
 * {@link #findOptimal()} returns all subsets of entities so that a metric defined in subclass is
 * minimized.
 */
public abstract class HeuristicSearch {
     ...
     * @param paths      Unsatisfiable paths identified in the constraint analysis
     * @param nSubopt    Number of suboptimal suggestions to report
     */
    ...
    protected class SearchNode {
         ...
         * @param index     the largest entity index in <code>entities</code> (avoid duplication)
         * @param remaining remaining paths to be solved
         * @param est       cost estimation
         */
        protected SearchNode(Set<Integer> entities, int index, Set<ConstraintPath> remaining,
                double est) {
    ...
    /**
     * @return Find a subset of candidates that 1) explains errors 2) minimizes the ranking metric
     */
    public Set<Explanation> findOptimal() {
        ...
        for (int i = 0; i < candidates.length; i++) {
            addSerchNode(queue, i,
                    new SearchNode(new HashSet<Integer>(), 0, paths.getPaths(), 0.0));
        }
    ...
    /**
     * @param ret  A set of explanations where the search node is added to if the node is an
     *             explanation
     * @param node A search node to be added if it is an explanation
     ...
     *
     * @param queue    The frontier of search tree where the new node is added to
     * @param candIdx  Index of the entity to be added to the previous search node
     *                 <code>previous</code>
     * @param previous The previous search node
     */
    abstract protected void addSerchNode(PriorityQueue<SearchNode> queue, int candIdx,
            SearchNode previous);
}
