...
public abstract class Element {
    ...
    /**
     * @return The code snippet corresponding to the element when available. Return element name
     * otherwise.
     */
    ...
    /**
     * String representation of `this` with double quotes replaced by single quotes.
     */
    public String toDotStringClean() {
        return toDotString().replace('"', '\'');
    }

    /**
     * @return True when the satisfiability of a path is trivial when it ends with the element
     * (e.g., a variable).
     */
    ...
    /**
     * Same constraint elements (except variables) at different positions are treated as different
     * elements to improve the precision of error diagnosis. However, when testing the partial
     * ordering on elements, this method must be called, so that all duplicated elements are
     * represented by the same base element, where the position is set to empty.
     */
    abstract public Element getBaseElement();

    /**
     * Try to unify the current element with a quantified-variable-free element e, by substituting
     * quantified variables to elements without quantified variables.
     *
     * @param e   The element to be unified with
     * @param map Substitutions made so far; when the unification succeeds, the new substitution
     *            introduced by the current unification is added to map
     * @return True if the unification succeeds, substitution is stored in map. Return false if the
     * unification fails
     */
     ...
     *
     * @param map A map from quantified variables to (quantified-variable-free) elements
    ...
    /**
     * Test if the current element can be unified with e, by substituting variables, assuming both
     * elements are quantified-variable-free.
     *
     * @param e   The element to be unified with
     * @param map Substitutions made so far; when the unification succeeds, the new substitution
     *            introduced by the current unification is added to map
