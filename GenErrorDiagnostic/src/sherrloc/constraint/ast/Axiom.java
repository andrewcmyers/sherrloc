...
/**
 * Axioms have the form of: \forall x,y. C_1 => C_2, where C is a conjunction of inequalities
 *
 */
public class Axiom {
    ...
    /**
     * Return all edges in the graph whose end nodes can potentially be unified with e1 and e2.
     * Update substitutions made in the unification when necessary.
     *
     * @param finder A saturated constraint graph where extra edges are to be inferred from axioms
     * @param maps   All possible substitutions so far
     * @return All edges in the graph whose end nodes can potentially be unified with e1 and e2.
     * Parameter maps is updated to remove infeasible substitutions, and make new substitutions for
     * the axiom when necessary
     */
    public List<EdgeMatch> findMatches(Element e1, Element e2, PathFinder finder,
            Map<QuantifiedVariable, Element> map) {
    ...
    public List<PremiseMatch> findMatchesInPremise(PathFinder finder) {
        ...
        matches.add(new PremiseMatch(new HashMap<QuantifiedVariable, Element>(), 0,
                new ArrayList<Evidence>(), true)); // add an empty
        // substitution
        for (Inequality ieq : premise) {
            ...
            while (!matches.isEmpty()) {
                ...
                for (EdgeMatch match : lst) {
                    ...
                    List<Evidence> evidences = new ArrayList<Evidence>(
                            pmatch.evidences);        // track evidences that lead to the inference
                    if ((finder.hasLeqEdge(n1, n2) && (ieq.r != Relation.EQ || finder.hasLeqEdge(n2,
                            n1)))) {
    ...
    /**
     * Return a list of nodes in the graph that matches an element, which in general, may contain
     * quantified variables
     *
     * @param e An element to be matched
     * @param m Substitutions collected so far
     * @return A list of nodes that matches element
     */
    private List<NodeMatch> matchOneNode(Element e, Map<QuantifiedVariable, Element> m,
            ConstraintGraph g) {
