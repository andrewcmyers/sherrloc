...
/**
 * A Hypothesis is a conjunction of inequalities and axioms. Since an hypothesis is used to reason
 * about provable inequalities in a constraint graph, relevant constraint elements are also added
 * into the hypothesis graph for a more complete reasoning.
 */
public class Hypothesis {
     ...
     *
     * @param ieq An inequality to be added
     */
     ...
     *
     * @param axiom Axioms to be added
     */
    ...
    /**
     * Set the parameter as axioms of the hypothesis; existing axioms are cleared
     *
     * @param axiom Axioms to be set
     */
     ...
     *
     * @param ele One element to be added
     */
     ...
     *
     * @param e An hypothesis to be added
     */
    ...
    /**
     * Return a fresh hypothesis where an inequality <code>e1 <= e2</code> is added to the current
     * hypothesis
     *
     * @param e1 Element on LHS
     * @param e2 Element on RHS
    ...
    /**
     * Test if the relation <code>e1</code> <= <code>e2</code> can be derived from the hypothesis
     *
     * @param p1 Element on LHS
     * @param p2 Element on RHS
...
//			}
            ...
            if (e1 instanceof ConstructorApplication
                ...
                if (((ConstructorApplication) e1).getCons().equals(
                        ((ConstructorApplication) e2).getCons()) && (e1.hasVars()
                        || e2.hasVars())) {
                    return true;
                }
            }
            ...
            if (e1 instanceof ConstructorApplication
                ...
                for (int i = 0; i < l1.size(); i++) {
                    if (variance.equals(Variance.POS) && !leq(l1.get(i), l2.get(i))) {
                        ...
                        return false;
                    } else if (variance.equals(Variance.NONE) && (!leq(l2.get(i), l1.get(i))
                            || !leq(
                            l1.get(i), l2.get(i)))) {
                        return false;
                    }
    ...
    /**
     * Saturate a hypothesis graph to test if <code>e1<=e2</code> can be inferred
     *
     * @param e1 Element on LHS
     * @param e2 Element on RHS
...
//			}
    ...
    /**
     * Since we are actually testing the satisfiability of the relation e1 <= e2, it is possible
     * that the relation is satisfiable, though the relation is not derivable from the hypotheses.
     * For instance, consider type-class constraints: list x <= Ord, with hypothesis \forall a . a
     * <= Ord; => (list a) <= Ord and Int <= Ord;;
     * <p>
     * We cannot *derive* that list x <= Ord, since x is a variable. But it's apparently satisfiable
     * when x = Int.
     * <p>
     * So here, we try to unify e1 and e2 with nodes in the constraint graph to test satisfiability
     */
