...
/**
 * This class identifies satisfiable and unsatisfiable constraints in a constraint graph.
 */
public class ConstraintAnalysisImpl implements ConstraintAnalysis {
    ...
    /**
     * @param isGenHypo True if SHErrLoc is inferring missing hypothesis
     * @param isVerbose True to collect data for evaluation
     * @param isRec     True if recursion is allowed
     */
    ...
    /**
     * Return an instance of constraint analysis. Currently, the only analysis implemented is
    ...
    public UnsatPaths genErrorPaths(ConstraintGraph graph) {
                        ...
                        if (finder.hasLeftEdge(node, m) && finder.hasLeftEdge(m, node)) {
                            ...
                            for (List<Edge> l1 : paths) {
                                for (List<Edge> l2 : finder.getLeftPaths(m, node)) {
                                    ...
                                    lst.addAll(l1);
                                    lst.addAll(l2);
                                    ConstraintPath path = new ConstraintPath(lst, finder,
                                            graph.getEnv());
                                    unsatPaths.addUnsatPath(path);
                                    if (DEBUG) {
                                        System.out.println("****** Infinite path ******");
        ...
        for (Axiom rule : graph.getRules()) {
            ...
            for (PremiseMatch pmatch : pmatches) {
                ...
                for (Inequality ieq : rule.getConclusion()) {
                    ...
                    if (m1 instanceof Application && (m2 instanceof Constructor
                            || m2 instanceof Function)) {
                    ...
                    } else if (m2 instanceof Application && (m1 instanceof Constructor
                            || m1 instanceof Function)) {
        ...
        for (Node start : allNodes) {
            for (Node end : allNodes) {
                ...
                if (!(start.getElement() instanceof JoinElement)
                        && !(end.getElement() instanceof MeetElement) && finder.hasLeqEdge(start,
                        end)) {
                    List<Edge> l = finder.getPath(start, end);
                    if (skolemCheck(start.getElement(), end.getElement())) {
                        testConsistency(start.getElement(), end.getElement(), l, graph, finder,
                                unsatPaths, false);
                    } else {
                        ConstraintPath path = new ConstraintPath(l, finder, graph.getEnv());
                        unsatPaths.addUnsatPath(path);
                ...
                if (needtest && !(end.getElement() instanceof JoinElement)
                        && !(start.getElement() instanceof MeetElement) && finder.hasLeqEdge(end,
                        start)) {
                    List<Edge> l = finder.getPath(end, start);
                    if (skolemCheck(start.getElement(), end.getElement())) {
                        testConsistency(end.getElement(), start.getElement(), l, graph, finder,
                                unsatPaths, false);
                    } else {
                        ConstraintPath path = new ConstraintPath(l, finder, graph.getEnv());
                        unsatPaths.addUnsatPath(path);
    ...
    void testConsistency(Element e1, Element e2, List<Edge> l, ConstraintGraph graph,
            PathFinder finder, UnsatPaths unsatPaths, boolean rec) {
        ...
        if (path.isInformative()) {
            if (path.isUnsatPath()) {
                if (isVerbose) {
                    System.out.println("Cannot unify " + path.getFirstElement() + " with "
                            + path.getLastElement());
                }
                if (DEBUG) {
    ...
    /**
     * Replace the variable in e with elements that flows into, or flows out of the variable, judged
     * by parameter ltor
     *
     * @param e    Element to be replaced
     * @param ltor True if the replacement uses elements that flows into variable in e
     */
    private List<VisitNode> expandElement(Application e, Element other, List<Edge> sofar,
            boolean ltor, PathFinder finder) {
    ...
    void expandGraph(Element e1, Element e2, List<Edge> l, ConstraintGraph graph, PathFinder finder,
            UnsatPaths unsatPaths) {
        List<VisitNode> toVisit = null;

        if (e1.hasVars() && e1 instanceof Application && (e2 instanceof Constructor
                || e2 instanceof Function)) {
            toVisit = expandElement((Application) e1, e2, l, true, finder);
        } else if (e2.hasVars() && e2 instanceof Application && (e1 instanceof Constructor
