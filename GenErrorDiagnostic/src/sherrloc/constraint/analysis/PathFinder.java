...
/**
 * The interface for path finders, which saturates a constraint graph so that derivable partial
 * orderings from constraints represented in graph are represented in the saturated graph
 */
public interface PathFinder {
     ...
     *
     * @param start Node on LHS
     * @param end   Node on RHS
     * @return A constraint path such that <code>start <= end</code> is derivable from constraints
     * along the path. Return null if no such path exists
     */
     ...
     * @param start Node on LHS
     * @param end   Node on RHS
     * @return Constraint paths such that <code>start LEFT end</code> is derivable from constraints
     * along the paths. Empty if no such path exists
     */
