...
public abstract class Graph {
    ...
    /**
     * This method takes a list of nodes, and collapse them into the first node. The connectivity to
     * the remaining of graph is maintained
     *
     * @param nodes A list of nodes to be collapsed
     */
    protected void collapse(List<Node> nodes, Set<Node>[] indeg, Map<Element, Node> eleToNode) {
        ...
        for (Edge edge : outEdges) {
            if (!hasLeqEdge(center, edge.to) && !center.equals(edge.to)) {
                leqEdges.get(center)
                        .put(edge.to, edge);                // add the out edge to center
        ...
        for (Edge edge : inEdges) {
            if (!hasLeqEdge(edge.from, center) && !edge.from.equals(center)) {
                leqEdges.get(edge.from)
                        .put(center, edge);                // add the out edge to center
                leqEdges.get(edge.from)
                        .remove(edge.to);                // remove the old edge from leqEdges
                indeg[edge.to.getIndex()].remove(edge.from);        // fix indeg
