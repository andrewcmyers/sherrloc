...
public class ConstraintPath {
    ...
    /**
     * @param edges     Edges forming the path
     * @param finder    A {@link PathFinder} that contains a saturated graph
     * @param globalEnv Global assumptions
     * @param cachedEnv Saturated hypothesis graphs for better performance
     */
    ...
    /**
     * @return The weakest hypothesis that makes the constraint path satisfiable. Return null for an
     * empty path
     */
    ...
    /**
     * A satisfiable path is one where all LEQ relations on nodes are satisfiable whenever there is
     * an LEQ edge on them
     *
    ...
    public boolean isInformative() {
        ...
        if (getFirst().isGray() || getLast().isGray()) {
            if (getFirstElement() instanceof ConstructorApplication
                    && getLastElement() instanceof ConstructorApplication) {
        ...
        for (int k = 0; k < edges.size(); k++) {
            ...
            if (needCmp) {
                ...
                if (!leqElements.empty()
                        ...
                        && !(leqElements.peek().equals(getFirstElement()) && eto.equals(
                        getLastElement()))) {
                    return false;
                } else if (!eto.hasVars()) {
                    leqElements.pop();
...
//      }
    ...
    /**
     * Increase the # satisfiable paths using constraint elements and constraints
     */
    ...
    /**
     * Mark all elements along the path as contributing to error. Only used for producing a complete
     * program slice that contributing to errors
     */
