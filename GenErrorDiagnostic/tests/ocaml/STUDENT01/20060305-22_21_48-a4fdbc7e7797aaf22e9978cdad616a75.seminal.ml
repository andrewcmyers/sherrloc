...
let rec handleAttr attrName attrValue lst = 
    print_string ("attr: " ^ attrName ^ "->" ^ attrValue ^ "\n");
    match lst with 
      [] -> text "end"
    | (attN, attV)::tl -> handleAttr attN attV tl

(* ##################### *)
let rec xmlToDoc xml = 
  match xml with
    [] -> text "foo"
  | Element(elementName, attributes, tl) -> 
        xmlToDoc tl
...
let xElement = Element("elName", ["attrName", "attrValue"; "attrName2", "attrValue2"], [])
let myDoc = xmlToDoc xElement
let _ = pretty_print (25) myDoc

(*let _ = xmlToDoc xElement
let _ = print_string "\n\n"*)
