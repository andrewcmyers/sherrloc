...
let rec handleAttr attrName attrValue lst = print_string ("attr: " ^ attrName ^ "->" ^ attrValue)
  ...
  match xml with
    Element(elementName, attributes, tl) -> 
        match attributes with 
          []                 -> print_string "empty"
        | (str1, str2)::tl2  -> handleAttr str1 str2 tl2

let xElement = Element("elName", [["attrName", "attrValue"]], [])
