...
let rec stmtToDoc stmt =
	match stmt with
	  Skip -> text "skip"
	| Assign(str, exp) ->
		group [nest 2 [text str ;
					   br ; text ":=" ;
					   br ; expToDoc exp] ]
	| Seq(stmt1, stmt2) ->
		(*smartcons ((group [stmtToDoc stmt1 ; breakWith "" ; text ";\n"]) stmtToDoc stmt2)*)
		smartcons ((stmtToDoc stmt1) (stmtToDoc stmt2))
	| If(exp, stmt1, stmt2) ->
		group [nest 3 [text "if" ; br ; text "(" ; br ; expToDoc exp ; br ; text ")" ;
					   br ; text "(" ; br ; stmtToDoc stmt1 ; br ; text ")" ;
					   br ; text "(" ; br ; stmtToDoc stmt2 ; br ; text ")"] ]
	| While(exp, stmt) ->
		group [group [text "while" ; breakWith " " ; expToDoc exp ; breakWith " " ; text "("] ;
			   group [text "\n    " ; nest 4 [stmtToDoc stmt] ; text "\n"] ;
			   text ")"]
