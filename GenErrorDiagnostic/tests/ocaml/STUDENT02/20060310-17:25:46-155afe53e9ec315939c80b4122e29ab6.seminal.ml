...
let rec format w k imdList = 
  ...
  | hd::tl	->
      match hd with
        (_,_,DocNil)			-> format w k tl
	  | (_,_,DocText s)			-> format w k tl
	  | (_,_,DocBreak s)		-> format w k tl
	  | (i,m,DocCons (dc1, dc2))->
		  ([i, m, dc1])::tl;
		  ([i, m, dc2])::tl;
		  format w k ([i, m, tl])
	  | (_,_,DocGroup d)		-> format w k tl
	  | (_,_,DocNest (i, d))	-> format w k tl
