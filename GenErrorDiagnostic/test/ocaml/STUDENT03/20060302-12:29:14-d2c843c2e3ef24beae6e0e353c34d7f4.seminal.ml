...
let rec print_list printfun l = 
  match l with 
      [] -> ()
    | x::[] -> (printfun x; print_string ".")
    | x::rest -> (printfun x; print_string ", "; print_list printfun rest)
...
let titlecase s = 
  let rec titlecase_helper src dest i prevChar =
    ...
    then titlecase_helper 
      src 
      (dest ^ (char2str (Char.uppercase src.[i])))
      (i+1) 
      src.[i]
    else titlecase_helper 
      src 
      (dest ^ (char2str (Char.lowercase src.[i])))
      (i+1)
      src.[i]
  in
...
let _ = print_string ((titlecase " ") ^ "\n")
let _ = print_string ((titlecase "this is a title in the right case") ^ "\n")

let histogram s =
  let count_char targetString wantedChar =
    fold_left (fun curCount -> (fun thisChar -> if thisChar == wantedChar then
      curCount + 1 else curCount)) 0 targetString
  in
  map (count_char (lowercase s)) "abcdefghijklmnopqrstuvwxyz"

let _ = print_int_list histogram "abcdefghijklmnopqrstuvwxyz"
(* 154,8-22 *)
