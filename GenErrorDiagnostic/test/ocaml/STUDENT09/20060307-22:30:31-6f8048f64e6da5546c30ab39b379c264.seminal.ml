...
let rec getAttListDoc attList acc = 
  match attList with
    [] -> acc
  | hd::tl -> match hd with
                hd::tl -> let ret = Pprintdata.group[Pprintdata.text " "; Pprintdata.text hd; Pprintdata.text "="; Pprintdata.text tl] in
                         getAttListDoc tl ret
let getXMLDoc eName attList xmlList =
    let attListDoc = getAttListDoc attList (Pprintdata.text "") in
    let xmlListDoc = getXMLListDoc xmlList in
    Pprintdata.group [Pprintdata.text "<"; Pprintdata.text eName; attListDoc; Pprintdata.text ">\n"; xmlListDoc; Pprintdata.text "</"; Pprintdata.text eName; Pprintdata.text ">"]
