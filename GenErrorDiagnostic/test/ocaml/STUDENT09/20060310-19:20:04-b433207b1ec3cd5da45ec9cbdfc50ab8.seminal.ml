...
##################################################### *)
let pretty w doc = 
  let stringDoc = format w 0 [(0,Flat, doc)] in
  sdocToString stringDoc
...
let test_prog filename = 
  let doc = stmtToDoc (get_prog_f filename) in
...
let _ =
let arg1 = Sys.argv.(0) in
let arg2 = Sys.argv.(1) in
pretty_print arg2 (get_prog_f 0)
